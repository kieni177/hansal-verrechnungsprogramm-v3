...
export class OrderDetailComponent implements OnInit {
  ...
  createInvoice(): void {
    ...
    this.invoiceService.getByOrderId(this.orderId).subscribe({
      next: (existingInvoice) => {
        if (existingInvoice) {
          this.notificationService.error('Für diese Bestellung existiert bereits eine Rechnung');
          return;
        }

        // Create the invoice if it doesn't exist
        this.invoiceService.createFromOrder(this.orderId!).subscribe({
          next: (invoice) => {
